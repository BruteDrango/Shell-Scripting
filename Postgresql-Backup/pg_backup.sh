...
# Purpose :- to take complete backup of the database
# Version :- I
# Created Date:- Thu Sep 28 09:00:35 PM IST 2023
# Modified Date:- 
	...
	case $1 in
		-c)
			if [ -r "$2" ]; then
				...
				${ECHO} "Unreachable config file \"$2\"" 1>&2
				exit 1
			fi
			;;
		*)
...
if [ "$BACKUP_USER" != "" -a "$(id -un)" != "BACKUP USER" ]; then
...
then
	...
	if ! pg_dumpall -g -h "$HOSTNAME" -U "$USERNAME" | gzip > $FINAL_BACKUP_DIR"globals".sql.gz.in_progress; then
		echo "[!!ERROR!!] Failed to produce global backup" >> 1>&2
	else
...
do
	...
	then
		...
		if ! pg_dump -Fp -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" | gzip > $FINAL_BACKUP_DIR"DATABASE".sql.gz.in_progress; 
		then
			echo "[!!ERROR!!] Failedto produce plain backup database $DATABASE" 1>&2
		else
	...
	then
		...
		if ! pg_dump -Fc -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" -f $FINAL_BACKUP_DIR"$DATABASE".custom.in_progress;
		then
	...
	then
		...
		if ! pg_dump -Ft -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" -f | gzip > $FINAL_BACKUP_DIR"DATABASE".sql.gz.in_progress;
		then
